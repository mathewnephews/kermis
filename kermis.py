...
pygame.display.set_caption("8-Bit Pretpark Arcade - Target Lock & Game Over")
...
# Mario Dying Sound (drie karakteristieke dalende tonen)
def play_mario_die_sound():
...
def perform_shot(shooter_indices, target_pos=None):
    ...
    for idx in valid_shooters:
        ...
        # Als er een specifiek doelwit is (X-toets target lock), schiet daarheen. Anders recht omhoog.
        end_pos = target_pos if target_pos else (cx + 4, 0)
        shot_lines.append([start_pos, end_pos, 3])

        # Check treffers op clowns
        for clown in clowns[:]:
            clown_rect = pygame.Rect(clown["x"] - 4, clown["y"] - 4, 20, 24)
            
            # Raakkans voor direct doelzoekend schot OF verticaal schot
...
def shoot_nearest_clown():
    """Vindt de dichtstbijzijnde clown die het verst gevorderd is (het laagst staat op het scherm)"""
    if not clowns:
        perform_shot([active_shooter_idx])
        return

    # Sorteer clowns op Y-positie (de onderste/dichtstbijzijnde eerst)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # Sneltoetsen a, r, c, u, p
            for idx, char in enumerate(characters):
                if event.key == char["key"]:
                    active_shooter_idx = idx
                    perform_shot([idx])

            # Z-toets: Alle levende figuren schieten tegelijk
            if event.key == pygame.K_z:
                perform_shot([0, 1, 2, 3, 4])

            # X-toets: Target lock op dichtstbijzijnde clown
    ...
    if not game_over:
        ...
        # Check of ALLE figuren dood zijn -> Game Over
        if all(not c["alive"] for c in characters):
            game_over = True
            pygame.mixer.stop()
            play_mario_die_sound()
