...
pygame.display.set_caption("8-Bit Pretpark Arcade - Dynamic Difficulty")
...
# Lijst van actieve schietlijnen
shot_lines = []

# Lijst voor meerdere gelijktijdige clowns
clowns = []
total_hits = 0
...
running = True
while running:
    # Bereken niveau / moeilijkheidsgraad per 10 hits
    level = total_hits // 10
    clown_base_speed = 0.4 + (level * 0.15)       # Begint traag (0.4) en versnelt geleidelijk
    max_clowns = min(1 + level, 5)                # Maximaal 5 clowns tegelijk
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # 'A' Toets: Alle 5 personages schieten tegelijk naar de muispositie
            ...
            # Klik op een personage om deze te selecteren
            hit_character = False
            ...
            # Enkelvoudig schot vanaf de actieve schutter
    ...
    # Clowns laten zakken & opruimen
    for clown in clowns[:]:
        clown["y"] += scroll_speed + clown_base_speed
        if clown["y"] > LOWER_ZONE_TOP:
            clowns.remove(clown)

    # Clowns spawnen zolang het maximum voor dit niveau nog niet is bereikt
    ...
    # Totale score & Huidig niveau bovenaan
    total_score = sum(c["score"] for c in characters)
    hud_text = font.render(f"SCORE: {total_score} | LEVEL: {level + 1} (MAX CLOWNS: {max_clowns})", True, BLACK)
