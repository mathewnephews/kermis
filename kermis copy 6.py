...
# Zones op het scherm
LOWER_ZONE_TOP = int(GAME_HEIGHT * 0.75)  # Zone voor wandelende figuren
GUIDE_BAR_TOP = int(GAME_HEIGHT * 0.88)   # Onderste balk voor namen & scores

game_surface = pygame.Surface((GAME_WIDTH, GAME_HEIGHT))
screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
pygame.display.set_caption("8-Bit Pretpark Arcade - Multi-Score Edition")
...
# 5 Personages met eigen individuele score ("score": 0)
characters = [
...
# Clown variabelen (clown loopt van top naar beneden)
clown_x, clown_y = 0, -20
clown_walk_speed = 0.8  # Loopsnelheid naar beneden
clown_visible = False
...
def spawn_clown():
    global clown_x, clown_y, clown_visible
    # Spawnt helemaal bovenaan het scherm op een willekeurige X-positie
...
while running:
    ...
    for event in pygame.event.get():
        ...
        # Toetsenbord selectie (1 t/m 5)
            ...
            if not hit_character:
                ...
                # Check of clown is neergeschoten
                if clown_visible:
                    clown_rect = pygame.Rect(clown_x - 4, clown_y - 4, 20, 24)
                    if clown_rect.collidepoint(gx, gy):
                        # Wijs punten toe aan het ACTIEVE personage dat geschoten heeft
    ...
    # Personages laten bewegen in de onderste zone
    ...
    # Clown beweegt van boven (top) naar beneden
    if clown_visible:
        clown_y += scroll_speed + clown_walk_speed
        # Als de clown onderaan het scherm verdwijnt
        if clown_y > LOWER_ZONE_TOP:
            clown_visible = False
    else:
        # Kans per frame dat er een nieuwe clown bovenaan inloopt
    ...
    for idx, char in enumerate(characters):
        ...
        # Cyan kader rondom de actieve schutter
    ...
    # Onderste gids-balk met individuele scores
    ...
    for idx, char in enumerate(characters):
        ...
        # Toont "Toets:Naam(Score)" bijv: 1:M@lb(20)
        ...
        x_offset += 83  # Afstand tussen spelers op de balk

    # Totale gecombineerde score bovenaan
    total_score = sum(c["score"] for c in characters)
    total_score_text = font.render(f"TOTAL SCORE: {total_score}", True, BLACK)
    game_surface.blit(total_score_text, (8, 6))
